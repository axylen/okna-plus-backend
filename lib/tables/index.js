...
async function initTables(clearTables = false) {
  ...
  let db_tables = (await mysql.query(`SHOW TABLES`)).map(
    tab => tab[`Tables_in_${DB_NAME}`],
  );
...
async function clearDatabase() {
  ...
  await Promise.all(
    table.allNames.map(name => mysql.query(`TRUNCATE ${name};`)),
  );
  await mysql.query(`SET FOREIGN_KEY_CHECKS=1;`);
  console.log(`[tables - clearDatabase] Удалены данные из таблиц`);
}
