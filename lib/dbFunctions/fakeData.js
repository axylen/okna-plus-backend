...
function generateFakeProduct(productsList) {
  ...
  const count =
    data.type !== 'window'
      ? 1
      : randomizeWeight([{ val: 1, weight: 5 }, { val: 2, weight: 3 }, { val: 3, weight: 1 }]);
...
async function generateFakeOrders(count) {
  ...
  await Promise.all(
    ordersProducts.map(async order => {
      const price = order.cart.reduce((acc, val) => acc + val.cost * val.count, 0);
      const { insertId } = await mysql.query(`INSERT INTO ${tables.name.orders} SET ? `, {
        ...order.order,
        price,
      });
      order.orderId = insertId;
    }),
  );
  ...
  await Promise.all(
    ordersProducts.map(async order => {
      return Promise.all(
        order.cart.map(prod => {
          return mysql.query(`INSERT INTO ${tables.name.order_products} SET ? `, {
            ...prod,
  ...
  console.log('[fakeData - generateFakeOrders] Добавлены фейковые заказы')
}
