...
async function generateFakeOrders(count) {
  ...
  const ordersArray = ordersProducts.map(order => {
    ...
    return objToArr(order.order);
  });

  const { insertId } = await mysql.query(
    `
    INSERT INTO ${tables.name.orders}
    (date, status, client, price) VALUES ?`,
    [ordersArray],
  );
  ...
  const { affectedRows: createdProducts } = await mysql.query(
    `
    INSERT INTO ${tables.name.order_products}
    (name, type, cost, count, area, params, order_id) VALUES ?`,
    [productsArray],
  );
